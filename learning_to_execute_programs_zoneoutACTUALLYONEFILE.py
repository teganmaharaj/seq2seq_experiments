...
zoneout_c_keep_prob = 0.05
zoneout_h_keep_prob = 0
num_symbols = len(SYMBOL_TO_IDX)
DEFAULT_LEARNING_RATE = 0.01
print('zoneout cells'+str(zoneout_c_keep_prob)+'states'+str(zoneout_h_keep_prob))
...
# state_keep_prob is the ZONEOUT PROBABILITY!
class ZoneoutWrapper(tf.nn.rnn_cell.RNNCell):
  """Operator adding zoneout to all states of the given cell."""

  def __init__(self, cell, state_keep_prob, is_training=True, seed=None):
    if not isinstance(cell, tf.nn.rnn_cell.RNNCell):
      raise TypeError("The parameter cell is not an RNNCell.")
    if (isinstance(state_keep_prob, float) and
        not (state_keep_prob >= 0.0 and state_keep_prob <= 1.0)):
      raise ValueError("Parameter state_keep_prob must be between 0 and 1: %d"
                       % state_keep_prob)
    self._cell = cell
  ...
  def __call__(self, inputs, state, scope=None):
    if isinstance(self.state_size, tuple) != isinstance(self._state_keep_prob, tuple):
      raise TypeError("Subdivided states need subdivided zoneouts.")
    if isinstance(self.state_size, tuple) and len(tuple(self.state_size)) != len(tuple(self._state_keep_prob)):
      raise ValueError("State and zoneout need equally many parts.")
    output, new_state = self._cell(inputs, state, scope)
    if isinstance(self.state_size, tuple):
      if self.is_training:
          new_state = tuple((1 - state_part_keep_prob) * tf.python.nn_ops.dropout(
                        new_state_part - state_part, (1 - state_part_keep_prob), seed=self._seed) + state_part
                            for new_state_part, state_part, state_part_keep_prob in zip(new_state, state, self._state_keep_prob))
      else:
          new_state = tuple(state_part_keep_prob * state_part + (1 - state_part_keep_prob) * new_state_part
                            for new_state_part, state_part, state_part_keep_prob in zip(new_state, state, self._state_keep_prob))
    else:
      if self.is_training:
          new_state = (1 - state_part_keep_prob) * tf.python.nn_ops.dropout(
                        new_state_part - state_part, (1 - state_part_keep_prob), seed=self._seed) + state_part
      else:
          new_state = state_part_keep_prob * state_part + (1 - state_part_keep_prob) * new_state_part
    ...
    def __init__(self,
        ...
        if num_layers > 1:
            cells = [ZoneoutWrapper(tf.nn.rnn_cell.LSTMCell(hidden_units, use_peepholes=peep, input_size=num_input_symbols, initializer=random_uniform(), state_is_tuple=True), 
                                    state_keep_prob=(zoneout_c_keep_prob, zoneout_h_keep_prob))]
            cells += [ZoneoutWrapper(tf.nn.rnn_cell.LSTMCell(hidden_units, use_peepholes=peep, input_size=hidden_units, initializer=random_uniform(), state_is_tuple=True),
                                     state_keep_prob=(zoneout_c_keep_prob, zoneout_h_keep_prob)) for _ in range(num_layers - 1)]
...
program_model.fit(program_generator, num_epochs=40, batches_per_epoch=20)
