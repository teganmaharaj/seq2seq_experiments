...
class ZoneoutWrapper(tf.nn.rnn_cell.RNNCell):
  ...
  def __init__(self, cell, state_keep_prob, seed=None):
  ...
  def __call__(self, inputs, state, scope=None):
    ...
    if isinstance(self.state_size, tuple):
      new_state = tuple(tf.python.nn_ops.dropout(new_state_part - state_part, state_part_keep_prob, seed=self._seed) + state_part
                        for new_state_part, state_part, state_part_keep_prob in zip(new_state, state, self._state_keep_prob))
    else:
      new_state = tf.python.nn_ops.dropout(new_state - state, self._state_keep_prob, seed=self._seed) + state
...
class Seq2SeqGraph():
    def __init__(self,
        ...
        if num_layers > 1:
            cells = [ZoneoutWrapper(rnn_cell.LSTMCell(hidden_units, use_peepholes=peep, input_size=num_input_symbols, initializer=random_uniform()), 
                                     state_keep_prob=(config.zoneout_c_keep_prob, config.zoneout_h_keep_prob))]
            cells += [ZoneoutWrapper(rnn_cell.LSTMCell(hidden_units, use_peepholes=peep, input_size=hidden_units, initializer=random_uniform()),
                                     state_keep_prob=(config.zoneout_c_keep_prob, config.zoneout_h_keep_prob)) for _ in range(num_layers - 1)]
            self.cell = rnn_cell.MultiRNNCell(cells)
        else:
            self.cell = ZoneoutWrapper(rnn_cell.LSTMCell(hidden_units, use_peepholes=peep, initializer=random_uniform()),
                                       state_keep_prob=(config.zoneout_c_keep_prob, config.zoneout_h_keep_prob))
